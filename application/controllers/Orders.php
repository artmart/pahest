...
class Orders extends CI_Controller {
    ...
    public function new_order()
    {
        ...
        for($i = 1; $i <= $post_number; $i++)
        {
            ...
            $preorder = 0;
            $presales = 0;
            if($this->input->post('preorder'.$i)){$preorder = 1;}
            ...
            if($this->input->post('daily'.$i))
            {
                ...
                $this->db->select('daily_order, preorder');
                ...
                $preorder_order = intval($daily_order[0]->preorder);
                
                $new_daily_order = $daily_order + $this->input->post('product_quantity'.$i);
                
                if($this->input->post('preorder'.$i)){
                    //$preorder = 1;
                    $preorder_order = $preorder_order + intval($this->input->post('product_quantity'.$i));
                }
                
                
                $this->db->where('id', $this->input->post('product_to_pick'.$i));
                $this->db->update('products', array('daily_order' => $new_daily_order, 'preorder' =>$preorder_order ));
            }else{
                $daily_price = 0;
                $sale_price = $this->input->post('product_price'.$i);
                
                $this->db->where('id', $this->input->post('product_to_pick'.$i));
                $this->db->select('quantity, sold_quantity, new_quantity, presales');
                $this->db->from('products');
                $query = $this->db->get();
                $res = $query->result();
                ...
                $presales = intval($res[0]->presales);
                ...
                if($this->input->post('preorder'.$i)){
                    $presales = $presales + intval($this->input->post('product_quantity'.$i));
                }
                
                $new_sold = $sold + intval($this->input->post('product_quantity'.$i));
                $this->db->where('id', $this->input->post('product_to_pick'.$i));
                $this->db->update('products', array('quantity' => $new_quantity, 'sold_quantity' => $new_sold, 'new_quantity' => $new_product_quantity, 'presales'=>$presales));
            }
    ...
    public function delete_order()
    {
        ...
        if($d_s == "sale")
        {
            ...
            //Added by Artak//
            $sold_quantity = intval($quantity[0]->sold_quantity)-$old_order;
            //////////////////

            $new_quantity = $quantity + $old_order;
            $this->db->where('id', $this->input->post('product_id'));
            $this->db->update('products', array('quantity' => $new_quantity, 'sold_quantity'=>$sold_quantity));
        }
        else
        {
            $this->db->where('id', $this->input->post('product_id'));
